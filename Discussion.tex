...
Based on the distribution of mixed modes, \citealt{gau14} reported that the oscillation pattern period spacing was typical of that of a star from the secondary red clump, i.e., a core-He-burning star that has not experienced an He flash. However, this conclusion was based on a period spacing of $\Delta \Pi \simeq 150 \ \rm{sec}$ (Mosser, private communication) which is difficult to measure robustly from a noisy oscillation spectrum. Red giant branch stars have smaller period spacings than red clump stars, and $\Delta \Pi = 150 \ \rm{sec}$, $\Delta \nu = 8.31 \ \mu \rm{Hz}$ puts the oscillating star on the very edge of the asteroseismic parameter space that defines the secondary red clump \cite{moss14}. As a result, we cannot say whether the oscillator in KIC 9246715 is on the red clump or still on the red giant branch.
...
While the asteroseismic $\log g$ measurement nearly agrees with those from ELC, all are some 0.3 dex lower than the spectroscopic $\log g$ values, as can be seen in Table \ref{table2}. This discrepancy is similar to the difference found for giant stars by \citet{hol15}. They investigate a large sample of stars from the ASPCAP (APOGEE Stellar Parameters and Chemical Abundances Pipeline) which have $\log g$ measured via spectroscopy and asteroseismology. They find that spectroscopic surface gravity measurements are roughly 0.2--0.3 dex too high for helium core burning (red clump) stars and roughly 0.1--0.2 dex too high for hydrogen shell burning (red giant branch) stars. \citet{hol15} speculate the difference may be partially due to a lack of treatment of stellar rotation, and derive an empirical calibration relation for a ``correct'' $\log g$ for red giant branch stars only. However, the stars in KIC 9246715 do not have particularly large rotational velocities (see Section \ref{parameters}), so we cannot dismiss this discrepancy so readily. We note that due to the broad nature of the mixed oscillation modes (see Section \ref{search}), we are unable to measure the oscillating star's core rotation rate.
